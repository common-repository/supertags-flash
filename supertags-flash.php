<?php
/*
	Plugin Name: SuperTags-Flash
	Description: SuperTags Flash is any easy to use widget that fits in the sidebar of your blog, it allows you to shows tags in nice cloud style flash object.
	Version: 1.5
	Author: Podz
...
function supertags_flash_install () {
	$newoptions = get_option('supertags_options');
	...
	add_option('supertags_options', $newoptions);
	// widget options
	$widgetoptions = get_option('supertags_widget');
	...
	$newoptions['args'] = '';
	$newoptions['mode'] = 'tags';
	add_option('supertags_widget', $newoptions);
}

// add the admin page
function supertags_flash_add_pages() {
	add_options_page('WP tags', 'WP tags', 8, __FILE__, 'supertags_flash_options');
}

// replace tag in content with tag cloud (non-shortcode version for WP 2.3.x)
function supertags_flash_init($content){
	if( strpos($content, '[SuperTags-Flash]') === false ){
		return $content;
	} else {
		$code = supertags_flash_createflashcode(false);
		$content = str_replace( '[SuperTags-Flash]', $code, $content );
		return $content;
	}
}

// template function
function supertags_flash_insert( $atts=NULL ){
	echo supertags_flash_createflashcode( false, $atts );
}

// shortcode function
function supertags_flash_shortcode( $atts=NULL ){
	return supertags_flash_createflashcode( false, $atts );
}

// piece together the flash code
function supertags_flash_createflashcode( $widget=false, $atts=NULL ){
	// get the options
	if( $widget == true ){
		$options = get_option('supertags_widget');
		$soname = "widget_so";
		$divname = "supertagswidgetcontent";
		// get compatibility mode variable from the main options
		$mainoptions = get_option('supertags_options');
		$options['compmode'] = $mainoptions['compmode'];
		$options['showwptags'] = $mainoptions['showwptags'];
	} else if( $atts != NULL ){
		$options = shortcode_atts( get_option('supertags_options'), $atts );
		$soname = "shortcode_so";
		$divname = "supertagscontent";
	} else {
		$options = get_option('supertags_options');
		$soname = "so";
		$divname = "supertagscontent";
	}
	// get the tag cloud...
	...
	if( function_exists('plugins_url') ){ 
		// 2.6 or better
		$movie = plugins_url('supertags-flash/tagcloud.swf');
		$path = plugins_url('supertags-flash/');
	} else {
		// pre 2.6
		$movie = get_bloginfo('wpurl') . "/wp-content/plugins/supertags-flash/tagcloud.swf";
		$path = get_bloginfo('wpurl')."/wp-content/plugins/supertags-flash/";
	}
...
// options page
function supertags_flash_options() {	
	$options = $newoptions = get_option('supertags_options');
	// if submitted, process results
	if ( $_POST["supertags_submit"] ) {
	...
	if ( $options != $newoptions ) {
		$options = $newoptions;
		update_option('supertags_options', $options);
	}
	// options form
	...
	echo '></input><br />Enabling this option switches the plugin to a different way of embedding Flash into the page. Use this if your page has markup errors or if you\'re having trouble getting SuperTags-Flash to display correctly. This affects both the shortcode version and the widget.</td></tr>';
	...
	// close stuff
	echo '<input type="hidden" name="supertags_submit" value="true"></input>';
	echo '</table>';
...
function supertags_flash_uninstall () {
	delete_option('tags_options');
	delete_option('tags_widget');
}


// widget
function widget_init_supertags_flash_widget() {
	// Check for required functions
	if (!function_exists('register_sidebar_widget'))
		return;

	function supertags_flash_widget($args){
	    extract($args);
		$options = get_option('supertags_widget');
		?>
			...
			<?php
				if( !stristr( $_SERVER['PHP_SELF'], 'widgets.php' ) ){
					echo supertags_flash_createflashcode(true);
				}
			?>
	...
	function supertags_flash_widget_control() {
		$options = $newoptions = get_option('supertags_widget');
		if ( $_POST["supertags_widget_submit"] ) {
			$newoptions['title'] = strip_tags(stripslashes($_POST["supertags_widget_title"]));
			$newoptions['width'] = strip_tags(stripslashes($_POST["supertags_widget_width"]));
			$newoptions['height'] = strip_tags(stripslashes($_POST["supertags_widget_height"]));
			$newoptions['tcolor'] = strip_tags(stripslashes($_POST["supertags_widget_tcolor"]));
			$newoptions['tcolor2'] = strip_tags(stripslashes($_POST["supertags_widget_tcolor2"]));
			$newoptions['hicolor'] = strip_tags(stripslashes($_POST["supertags_widget_hicolor"]));
			$newoptions['bgcolor'] = strip_tags(stripslashes($_POST["supertags_widget_bgcolor"]));
			$newoptions['speed'] = strip_tags(stripslashes($_POST["supertags_widget_speed"]));
			$newoptions['trans'] = strip_tags(stripslashes($_POST["supertags_widget_trans"]));
			$newoptions['distr'] = strip_tags(stripslashes($_POST["supertags_widget_distr"]));
			$newoptions['args'] = strip_tags(stripslashes($_POST["supertags_widget_args"]));
			$newoptions['mode'] = strip_tags(stripslashes($_POST["supertags_widget_mode"]));
		}
		if ( $options != $newoptions ) {
			$options = $newoptions;
			update_option('supertags_widget', $options);
		}
		...
		?>
			<p><label for="supertags_widget_title"><?php _e('Title:'); ?> <input class="widefat" id="supertags_widget_title" name="supertags_widget_title" type="text" value="<?php echo $title; ?>" /></label></p>
			<p><label for="supertags_widget_width"><?php _e('Width:'); ?> <input class="widefat" id="supertags_widget_width" name="supertags_widget_width" type="text" value="<?php echo $width; ?>" /></label></p>
			<p><label for="supertags_widget_height"><?php _e('Height:'); ?> <input class="widefat" id="supertags_widget_height" name="supertags_widget_height" type="text" value="<?php echo $height; ?>" /></label></p>
			<p><label for="supertags_widget_tcolor"><?php _e('Tag color:'); ?> <input class="widefat" id="supertags_widget_tcolor" name="supertags_widget_tcolor" type="text" value="<?php echo $tcolor; ?>" /></label></p>
			<p><label for="supertags_widget_tcolor2"><?php _e('Optional second color for gradient:'); ?> <input class="widefat" id="supertags_widget_tcolor2" name="supertags_widget_tcolor2" type="text" value="<?php echo $tcolor2; ?>" /></label></p>
			<p><label for="supertags_widget_hicolor"><?php _e('Optional highlight color:'); ?> <input class="widefat" id="supertags_widget_hicolor" name="supertags_widget_hicolor" type="text" value="<?php echo $hicolor; ?>" /></label></p>
			<p><label for="supertags_widget_bgcolor"><?php _e('Background color:'); ?> <input class="widefat" id="supertags_widget_bgcolor" name="supertags_widget_bgcolor" type="text" value="<?php echo $bgcolor; ?>" /></label></p>
			<p><label for="supertags_widget_trans"><input class="checkbox" id="supertags_widget_trans" name="supertags_widget_trans" type="checkbox" value="true" <?php if( $trans == "true" ){ echo ' checked="checked"'; } ?> > Background transparency</label></p>
			<p><label for="supertags_widget_speed"><?php _e('Speed:'); ?> <input class="widefat" id="supertags_widget_speed" name="supertags_widget_speed" type="text" value="<?php echo $speed; ?>" /></label></p>
			<p><label for="supertags_widget_distr"><input class="checkbox" id="supertags_widget_distr" name="supertags_widget_distr" type="checkbox" value="true" <?php if( $distr == "true" ){ echo ' checked="checked"'; } ?> > Distribute evenly on sphere</label></p>
			<p>
				<input class="radio" id="supertags_widget_mode" name="supertags_widget_mode" type="radio" value="tags" <?php if( $mode == "tags" ){ echo ' checked="checked"'; } ?> > Tags<br />
				<input class="radio" id="supertags_widget_mode" name="supertags_widget_mode" type="radio" value="cats" <?php if( $mode == "cats" ){ echo ' checked="checked"'; } ?> > Categories<br />
				<input class="radio" id="supertags_widget_mode" name="supertags_widget_mode" type="radio" value="both" <?php if( $mode == "both" ){ echo ' checked="checked"'; } ?> > Both
			</p>
			<p><label for="supertags_widget_args"><?php _e('wp_tag_cloud parameters:'); ?> <input class="widefat" id="supertags_widget_args" name="supertags_widget_args" type="text" value="<?php echo $args; ?>" /></label></p>
			<input type="hidden" id="supertags_widget_submit" name="supertags_widget_submit" value="1" />
		<?php
	}
	
	register_sidebar_widget( "SuperTags-Flash", supertags_flash_widget );
	register_widget_control( "SuperTags-Flash", "supertags_flash_widget_control" );
}

// Delay plugin execution until sidebar is loaded
add_action('widgets_init', 'widget_init_supertags_flash_widget');

// add the actions
add_action('admin_menu', 'supertags_flash_add_pages');
register_activation_hook( __FILE__, 'supertags_flash_install' );
register_deactivation_hook( __FILE__, 'supertags_flash_uninstall' );
if( function_exists('add_shortcode') ){
	add_shortcode('SuperTags-Flash', 'supertags_flash_shortcode');
	add_shortcode('SuperTags-Flash', 'supertags_flash_shortcode');
} else {
	add_filter('the_content','supertags_flash_init');
}
